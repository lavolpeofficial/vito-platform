  ...
  return (
    <main>
      ...
      <section className="module-heading">
        <div className="module-icon">WF</div>
        <div><span className="eyebrow">Governed execution</span><h1>Workflow Cockpit</h1><p>Persistierte Runs beobachten, Step-Grenzen verstehen und ausschließlich die vom Backend freigegebene nächste Aktion auslösen.</p></div>
      </section>
      <div className="boundary-notice"><span className="system-dot" /><div><strong>Human gates bleiben harte Grenzen</strong><p>Das Cockpit startet, resumiert oder führt Agent-Steps aus. HUMAN_REVIEW_REQUIRED und Release-Gates werden niemals automatisch bestätigt.</p></div></div>
      ...
      <section className="workflow-grid">
        <div className="workflow-panel">
          <span className="eyebrow">01 · Select</span><h2>Run öffnen</h2>
          <form method="get" className="workflow-search"><input name="run" defaultValue={requestedRun} maxLength={512} placeholder="Workflow Run ID" required /><button type="submit">Beobachten</button></form>
          <p className="workflow-muted">Alternativ einen Run aus der aktuellen Attention-Liste auswählen.</p>
          <div className="workflow-attention-list">{attention.length === 0 ? <p className="workflow-muted">Keine Attention-Runs gemeldet.</p> : attention.map((run) => <AttentionRun key={run.id} run={run} />)}</div>
        </div>
...
function AttentionRun({ run }: Readonly<{ run: OperationsAttentionRun }>) {
  return <a className="workflow-attention" href={`/workflows?run=${encodeURIComponent(run.id)}`}><div><strong>{run.status}</strong><span>{run.currentStepType ?? 'no current step'}</span></div><small>{run.blockReasonCode ?? run.failureReasonCode ?? run.id}</small></a>;
}

function ActionPanel({ snapshot }: Readonly<{ snapshot: WorkflowSnapshot }>) {
  const actionable = snapshot.nextAction === 'START_RUN' || snapshot.nextAction === 'RESUME_RUN' || snapshot.nextAction === 'EXECUTE_CURRENT_STEP';
  const labels: Readonly<Record<string, string>> = { START_RUN: 'Workflow starten', RESUME_RUN: 'Workflow resumieren', EXECUTE_CURRENT_STEP: 'Aktuellen Agent-Step ausführen', HUMAN_REVIEW_REQUIRED: 'Human Review erforderlich', NONE: 'Keine Aktion' };
  return (
    <div className="workflow-action-card">
      <div className="workflow-state-row"><span>Status</span><strong>{snapshot.status}</strong></div><div className="workflow-state-row"><span>Boundary</span><strong>{snapshot.boundary}</strong></div><div className="workflow-state-row"><span>Next action</span><strong>{snapshot.nextAction}</strong></div>
      {snapshot.blockReasonCode ? <div className="workflow-reason">Block: {snapshot.blockReasonCode}</div> : null}{snapshot.failureReasonCode ? <div className="workflow-reason">Failure: {snapshot.failureReasonCode}</div> : null}
      {actionable ? <form action={workflowAction}><input type="hidden" name="workflowRunId" value={snapshot.workflowRunId} /><input type="hidden" name="action" value={snapshot.nextAction} /><button className="primary-button" type="submit">{labels[snapshot.nextAction]}</button></form> : <div className="workflow-governance-stop"><strong>{labels[snapshot.nextAction] ?? snapshot.nextAction}</strong><p>Keine mutierende Aktion wird angeboten. Freigaben bleiben außerhalb dieses Cockpits.</p></div>}
    </div>
  );
}

function SnapshotView({ snapshot }: Readonly<{ snapshot: WorkflowSnapshot }>) {
  return (
    <section className="workflow-detail">
      <div className="workflow-section-head"><div><span className="eyebrow">03 · Observer</span><h2>Persistierter Run</h2></div><span className="status-chip">READ ONLY SNAPSHOT</span></div>
      <div className="workflow-summary"><Metric label="Run" value={snapshot.workflowRunId} /><Metric label="Current step" value={snapshot.currentStepType ?? '—'} /><Metric label="Correction loops" value={`${snapshot.correctionLoopCount} / ${snapshot.maxCorrectionLoops}`} /><Metric label="Observed" value={formatDate(snapshot.observedAt)} /></div>
      <h3>Steps</h3><div className="workflow-step-list">{snapshot.steps.map((step) => <article className="workflow-step" key={step.id}><div><strong>{step.stepType}</strong><span>attempt {step.attemptNumber}</span></div><span className="workflow-step-status">{step.status}</span><small>{formatDate(step.startedAt)}{step.finishedAt ? ` → ${formatDate(step.finishedAt)}` : ''}</small></article>)}</div>
      ...
      {snapshot.timelineTruncated ? <p className="workflow-muted">Timeline auf die serverseitige Maximalzahl begrenzt.</p> : null}
    </section>
  );
}
