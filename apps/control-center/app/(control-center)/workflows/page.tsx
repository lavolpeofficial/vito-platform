...
function ActionPanel({ snapshot }: Readonly<{ snapshot: WorkflowSnapshot }>) {
  const actionable = snapshot.nextAction === 'START_RUN' || snapshot.nextAction === 'RESUME_RUN' || snapshot.nextAction === 'EXECUTE_CURRENT_STEP' || snapshot.nextAction === 'APPROVE_HUMAN_RELEASE';
  const labels: Readonly<Record<string, string>> = { START_RUN: 'Workflow starten', RESUME_RUN: 'Workflow resumieren', EXECUTE_CURRENT_STEP: 'Aktuellen Agent-Step ausführen', APPROVE_HUMAN_RELEASE: 'Human Release ausdrücklich freigeben', HUMAN_REVIEW_REQUIRED: 'Human Review erforderlich', NONE: 'Keine Aktion' };
  const isReleaseApproval = snapshot.nextAction === 'APPROVE_HUMAN_RELEASE';
  return <div className="workflow-action-card"><div className="workflow-state-row"><span>Status</span><strong>{snapshot.status}</strong></div><div className="workflow-state-row"><span>Boundary</span><strong>{snapshot.boundary}</strong></div><div className="workflow-state-row"><span>Next action</span><strong>{snapshot.nextAction}</strong></div>{snapshot.blockReasonCode ? <div className="workflow-reason">Block: {snapshot.blockReasonCode}</div> : null}{snapshot.failureReasonCode ? <div className="workflow-reason">Failure: {snapshot.failureReasonCode}</div> : null}{isReleaseApproval ? <div className="workflow-reason"><strong>Explizite Human-Freigabe:</strong> Dieser Klick bestätigt nur den HUMAN_RELEASE_GATE und setzt RELEASE_EXECUTION bereit. Er startet keine Release-Ausführung.</div> : null}{actionable ? <form action={workflowAction}><input type="hidden" name="workflowRunId" value={snapshot.workflowRunId} /><input type="hidden" name="action" value={snapshot.nextAction} /><button className="primary-button" type="submit">{labels[snapshot.nextAction]}</button></form> : <div className="workflow-governance-stop"><strong>{labels[snapshot.nextAction] ?? snapshot.nextAction}</strong><p>Keine mutierende Aktion wird angeboten. Andere Human Reviews bleiben harte Governance-Grenzen.</p></div>}</div>;
}
